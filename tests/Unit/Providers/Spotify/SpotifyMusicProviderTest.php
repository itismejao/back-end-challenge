...
namespace Tests\Unit\Providers\Spotify;
...
class SpotifyMusicProviderTest extends TestCase
{
    ...
    public function it_returns_null_when_no_results_found(): void
    {
        ...
        $result = $this->provider->searchByIsrc('XXXX00000000');
    ...
    public function it_maps_track_data_correctly(): void
    {
        ...
        $this->assertSame('NO1R42509310', $result->isrc);
        $this->assertSame(215491, $result->durationMs);
        $this->assertTrue($result->explicit);
        $this->assertSame(1, $result->discNumber);
        $this->assertSame(1, $result->trackNumber);
        $this->assertSame('7qaLSS4e5luH7meEYGu7NM', $result->externalId);
        $this->assertSame('https://open.spotify.com/track/7qaLSS4e5luH7meEYGu7NM', $result->externalUrl);
    }
    ...
    public function it_maps_album_data_correctly(): void
    {
        ...
        $this->assertSame('2025-11-28', $result->album->releaseDate);
        $this->assertSame(1, $result->album->totalTracks);
        $this->assertCount(3, $result->album->images);
        $this->assertSame('5ZOUvcjuQWsNyohlotwRRZ', $result->album->externalId);
    }
    ...
    public function it_maps_artists_correctly(): void
    {
        ...
        $this->assertSame('3gXdxjEAOYqbKYDYRGuxUS', $result->artists[0]->externalId);
        $this->assertSame('Redmenn', $result->artists[1]->name);
    }
    ...
    public function it_collects_available_markets(): void
    {
        ...
        $result = $this->provider->searchByIsrc('NO1R42509310', ['BR', 'US']);

        $this->assertSame(['BR', 'US'], $result->availableMarkets);
    }
    ...
    public function it_retries_on_401_with_fresh_token(): void
    {
        ...
        Http::fake([
            ...
            'api.spotify.com/v1/search*' => function () use (&$callCount) {
                $callCount++;
                if ($callCount === 1) {
                    return Http::response(['error' => ['status' => 401, 'message' => 'Expired']], 401);
                }

                return Http::response($this->spotifySearchResponse());
            },
        ]);
    ...
    public function it_normalizes_unknown_album_type_to_album(): void
    {
        ...
        $this->assertSame('album', $result->album->albumType);
    }
