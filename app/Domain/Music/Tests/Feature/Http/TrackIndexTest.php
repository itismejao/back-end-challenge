...
namespace Music\Tests\Feature\Http;
...
class TrackIndexTest extends TestCase
{
    ...
    private function createTrack(
        string $name = 'Test Track', string $isrc = 'TEST12345678',
        string $externalId = 'ext1', int $durationMs = 215491, array $markets = [],
    ): Track {
        $album = Album::firstOrCreate(['name' => 'Test Album'], [
        ...
        if (! empty($markets)) { $track->availableMarkets()->sync($markets); }
        $track->externalIds()->create([
            'provider_code' => 'spotify', 'external_id' => $externalId,
            'external_url' => 'https://open.spotify.com/track/' . $externalId, 'synced_at' => now(),
        ]);
        return $track;
    }
}
